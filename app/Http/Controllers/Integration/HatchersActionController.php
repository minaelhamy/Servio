...
class HatchersActionController extends Controller
{
    ...
    private function generateWebsiteMediaAssets(array $payload): array
    {
        ...
        return array_values(array_filter([
            ...
            $this->slotMediaPayload('section_one', 'section one banner', $features),
            ...
            $this->slotMediaPayload('category', 'category image', $features ?? $hero),
            $this->slotMediaPayload('service_primary', 'service primary image', $hero ?? $features),
            $this->slotMediaPayload('service_detail', 'service detail image', $features ?? $proof ?? $hero),
            $this->slotMediaPayload('service_support', 'service support image', $action ?? $story ?? $hero),
            $this->slotMediaPayload('testimonial_primary', 'testimonial image', $proof ?? $story ?? $hero),
            $this->slotMediaPayload('gallery_primary', 'gallery image one', $story ?? $hero),
            $this->slotMediaPayload('gallery_secondary', 'gallery image two', $features ?? $hero),
            $this->slotMediaPayload('gallery_tertiary', 'gallery image three', $action ?? $proof ?? $hero),
            $this->slotMediaPayload('why_choose_item', 'why choose us image', $story ?? $features ?? $hero),
        ]));
    }
    ...
    private function websiteMediaQueryPlan(array $payload): array
    {
        ...
        return [
            ...
            ['slot_key' => 'features', 'query' => trim($title . ' professional service detail'), 'fallback_queries' => [trim($featureTitle), trim($title . ' quality service')]],
            ...
            ['slot_key' => 'action', 'query' => trim($title . ' booking appointment action'), 'fallback_queries' => [trim($title . ' contact booking')]],
        ];
    }
